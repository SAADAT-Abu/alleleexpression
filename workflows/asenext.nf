...
workflow ASENEXT {
    ...
    // Extract just the reads for FastQC and STAR
    ch_vcf_input
    ...
    // Create dummy channels for STAR_ALIGN
    ...
    // STAR_ALIGN requires 7 inputs based on your module version:
    // reads, index, gtf, star_ignore_sjdbgtf, seq_platform, seq_center
    STAR_ALIGN (
        ch_reads,                  // reads
    ...
    FILTER_WASP_READS (
        STAR_ALIGN.out.bam.map { meta, bam -> [ meta, bam, [] ] },  // input with empty index
    ...
    // Create dummy channels for BCFTOOLS_VIEW
    ch_dummy_regions = Channel.value(file("${workflow.projectDir}/assets/dummy_regions.txt"))
    ch_dummy_targets = Channel.value(file("${workflow.projectDir}/assets/dummy_targets.txt"))
    ch_dummy_samples = Channel.value(file("${workflow.projectDir}/assets/dummy_samples.txt"))
    
    // Extract chromosome from VCF
    // BCFTOOLS_VIEW requires 5 inputs based on your module version:
    // vcf with index, regions, targets, samples
    BCFTOOLS_VIEW (
        PREPARE_VCF.out.vcf_beagle,             // vcf with index
        ch_dummy_regions,                       // regions
        ch_dummy_targets,                       // targets
        ch_dummy_samples                        // samples
    )
    ...
    BEAGLE5_BEAGLE (
        ...
        ch_dummy_exclsamples,                   // exclsamples
        ch_dummy_exclmarkers                    // exclmarkers
    )
